"""Visualize comment sentiment for the top 10 most-commented posts."""
...
DEFAULT_INPUT = (
    ...
    / "data"
    / "processed"
    / "comment_sentiment_analysis_top10"
    / "comments_sentiment_top10.csv"
)
DEFAULT_OUTPUT_DIR = (
    PROJECT_ROOT / "data" / "processed" / "comment_sentiment_analysis_top10" / "visualizations"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize Top10 comment sentiment analysis.")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--min-comments-for-topic-year", type=int, default=20)
...
def plot_post_comment_sentiment_matrix(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("Post Sentiment x Comment Sentiment | Top10", fontsize=15, fontweight="bold")
    ...
    path = output_dir / "post_comment_sentiment_matrix_top10.png"
    save_fig(path)
    return path
...
def plot_comment_negative_ratio_by_post_sentiment(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("Comment Negative Ratio by Post Sentiment | Top10", fontsize=15, fontweight="bold")
    ax.set_xlabel("post_sentiment")
    ...
    path = output_dir / "comment_negative_ratio_by_post_sentiment_top10.png"
    save_fig(path)
    return path
...
def plot_topic_year_negative_ratio_heatmap(df: pd.DataFrame, output_dir: Path, min_comments: int) -> Path:
    ...
    topic_order = (
        df.groupby("topic").size().sort_values(ascending=False).index.astype(str).tolist()
    )
    table = (
    ...
    ax.set_title("Topic-Year Negative Comment Ratio | Top10", fontsize=15, fontweight="bold")
    ...
    path = output_dir / "topic_year_negative_ratio_heatmap_top10.png"
    save_fig(path)
    return path
...
def plot_user_activity_sentiment_scatter(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    ax.set_title("User Activity x Dominant Comment Sentiment | Top10", fontsize=15, fontweight="bold")
    ...
    path = output_dir / "user_activity_sentiment_scatter_top10.png"
    save_fig(path)
    return path


def write_summary(output_dir: Path, paths: list[Path], df: pd.DataFrame) -> Path:
    path = output_dir / "comment_sentiment_top10_visualization_summary.md"
    counts = df["comment_sentiment_label"].value_counts().reindex(SENTIMENT_ORDER, fill_value=0)
    lines = [
        "# Top10 Comment Sentiment Visualizations",
        "",
