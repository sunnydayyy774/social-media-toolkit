"""Run sentiment analysis for comments under the top 10 most-commented posts."""
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "data" / "processed" / "comment_sentiment_analysis_top10"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Top10 comment sentiment analysis.")
    ...
    parser.add_argument("--prepare-only", action="store_true", help="Only write Top10 input files, do not load the model.")
    parser.add_argument("--dry-run", action="store_true", help="Print counts without writing sentiment rows.")
    parser.add_argument("--overwrite", action="store_true", help="Replace existing comments_sentiment_top10.csv.")
...
def read_post_sentiment(path: Path) -> pd.DataFrame:
    ...
    required = {"aweme_id", "sentiment_label", "sentiment_score", "sentiment_status"}
    ...
    return post_sentiment.rename(
        columns={
            "sentiment_label": "post_sentiment_label",
            "sentiment_score": "post_sentiment_score",
        }
    )[["aweme_id", "post_sentiment_label", "post_sentiment_score"]]


def build_inputs(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    post_required = {"aweme_id", "create_time", "topic", "text", "comment_count", "likes", "include_in_analysis"}
    ...
    posts = posts[as_bool_true(posts["include_in_analysis"])].copy()
    posts["comment_count_num"] = as_numeric(posts["comment_count"])
    top_videos = posts.sort_values("comment_count_num", ascending=False).head(args.top_n).copy()
    top_videos = top_videos.merge(post_sentiment, on="aweme_id", how="left")
    top_videos["post_sentiment_label"] = top_videos["post_sentiment_label"].fillna("missing")
    top_videos["post_sentiment_score"] = top_videos["post_sentiment_score"].fillna("")
    top_videos = top_videos[TOP_VIDEO_COLUMNS].copy()

    top_aweme_ids = set(top_videos["aweme_id"].astype(str))
    selected_comments = comments[
        as_bool_true(comments["include_in_analysis"]) & comments["aweme_id"].astype(str).isin(top_aweme_ids)
    ].copy()
    selected_comments = selected_comments.rename(columns={"user_uid": "user_id"})
    selected_comments = selected_comments.merge(
        top_videos[["aweme_id", "topic", "post_sentiment_label"]],
        on="aweme_id",
        how="left",
    )
    selected_comments = selected_comments[COMMENT_INPUT_COLUMNS].copy()
    return top_videos, selected_comments
...
def write_input_files(output_dir: Path, top_videos: pd.DataFrame, selected_comments: pd.DataFrame) -> tuple[Path, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    top_path = output_dir / "top10_video_list.csv"
    comments_path = output_dir / "comments_for_sentiment_top10.csv"
    top_videos.to_csv(top_path, index=False, encoding="utf-8-sig")
    selected_comments.to_csv(comments_path, index=False, encoding="utf-8-sig")
    return top_path, comments_path
...
def main() -> None:
    ...
    top_videos, selected_comments = build_inputs(args)
    top_path, comments_input_path = write_input_files(output_dir, top_videos, selected_comments)

    output_path = output_dir / "comments_sentiment_top10.csv"
    ...
    logging.info("Top videos selected: %s", len(top_videos))
    logging.info("Top video list: %s", top_path)
    logging.info("Top10 comments input: %s", comments_input_path)
    ...
    logging.info("Top10 comment sentiment complete: %s", output_path)
